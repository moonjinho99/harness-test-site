...
// Self-check — run `npx tsx src/lib/products.ts` to verify mock fallback wiring.
if (require.main === module) {
  ;(async () => {
    const all = await getProducts()
    console.assert(all.products.length === 12, "expected 12 products")
    console.assert(all.total === 12, "expected total=12")

    const clothing = await getProducts({ categorySlug: "clothing" })
    console.assert(
      clothing.products.every((p) => p.categorySlug === "clothing"),
      "categorySlug filter broken",
    )

    const legacyFiltered = await getProducts({ category: "의류" })
    console.assert(
      legacyFiltered.products.every((p) => p.category === "의류"),
      "legacy category filter broken",
    )

    const asc = await getProducts({ sortBy: "price_asc", limit: 20 })
    for (let i = 1; i < asc.products.length; i++) {
      console.assert(
        asc.products[i - 1].price <= asc.products[i].price,
        "price_asc order broken",
      )
    }

    const one = await getProductBySlug("premium-linen-shirt")
    console.assert(one?.name === "프리미엄 린넨 셔츠", "getProductBySlug broken")
    const none = await getProductBySlug("nope")
    console.assert(none === null, "missing slug should be null")

    const cats = await getCategories()
    console.assert(cats.length === 6, "expected 6 categories")

    console.log("products.ts self-check OK")
  })()
}
